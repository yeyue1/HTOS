...
static void fill_program_header(
		elf_phdr_t *program_header, elf_word type, elf_addr vaddr, elf_off filesz, elf_off memsz, elf_word flag)
{
...
static int32_t minidump_memarea(struct thread_info_ops *ops, int32_t idx, uint32_t *addr, uint32_t *memlen)
{
