...
typedef struct {
  ...
  uint8_t
      desc[MCOREDUMP_PRSTATUS_SIZE]; /* descriptor of this note entry type */
} elf_note_prstatus_t;
...
typedef struct {
  ...
  elf_word descsz; /* Size of the note descriptor */
  elf_word type;   /* Interpretation of the descriptor */
  char name[MCD_ALIGN(sizeof("LINUX"),
                      4)]; /* The only support name is "LINUX" */
  uint8_t
      desc[MCOREDUMP_FPREGSET_SIZE]; /* descriptor of this note entry type */
...
static void fill_note_prstatus(elf_note_prstatus_t *prstatus,
                               core_regset_type *regset) {
  fill_note_base((elf_note_base_t *)prstatus, "CORE", NT_PRSTATUS,
                 sizeof(prstatus->desc));
  fill_note_prstatus_desc(&prstatus->desc[0], regset);
}

/*  fill FP registers struct in note segment */
static void fill_note_vfp_regset(elf_note_fpreg_entry_t *prstatus,
                                 fp_regset_type *regset) {
  fill_note_base((elf_note_base_t *)prstatus, "LINUX", NT_ARM_VFP,
                 sizeof(prstatus->desc));
...
static void fill_one_threads_regset(core_regset_type *core_regset,
                                    fp_regset_type *fp_regset) {
  fill_note_prstatus(&m_ctx.tmp.prstatus, core_regset);
  m_ctx.writeout_func((uint8_t *)&m_ctx.tmp.prstatus,
                      sizeof(elf_note_prstatus_t));

  if (m_ctx.with_fp) {
    fill_note_vfp_regset(&m_ctx.tmp.note_fp, fp_regset);
    m_ctx.writeout_func((uint8_t *)&m_ctx.tmp.note_fp,
...
void mcd_gen_coredump(struct thread_info_ops *ops) {
  ...
  if (m_ctx.with_fp)
    note_size = (sizeof(elf_note_prstatus_t) + sizeof(elf_note_fpreg_entry_t)) *
                ops->get_threads_count(ops);
  else
    note_size = sizeof(elf_note_prstatus_t) * ops->get_threads_count(ops);
  fill_program_header(&m_ctx.tmp.program_header, PT_NOTE, 0, note_size, 0,
                      PF_R);
  ...
  for (int i = 0; i < ops->get_memarea_count(ops); i++) {
    ...
    fill_program_header(&m_ctx.tmp.program_header, PT_LOAD, addr, memlen,
                        memlen, PF_R | PF_W);
    m_ctx.writeout_func((uint8_t *)&m_ctx.tmp.program_header,
                        sizeof(elf_phdr_t));
  }
...
int32_t mcd_corefile_size(struct thread_info_ops *ops) {
  ...
  if (m_ctx.with_fp)
    elf_size += (sizeof(elf_note_prstatus_t) + sizeof(elf_note_fpreg_entry_t)) *
                ops->get_threads_count(ops);
  else
...
static int32_t minidump_thr_cnts(struct thread_info_ops *ops) { return 1; }

static int32_t minidump_cur_idx(struct thread_info_ops *ops) { return 0; }

static void minidump_thr_rset(struct thread_info_ops *ops, int32_t idx,
                              core_regset_type *core_regset,
                              fp_regset_type *fp_regset) {
  mcd_memcpy(core_regset, get_cur_core_regset_address(),
             sizeof(core_regset_type));
  mcd_memcpy(fp_regset, get_cur_fp_regset_address(), sizeof(fp_regset_type));
}

static int32_t minidump_mem_cnts(struct thread_info_ops *ops) { return 1; }
...
fp_regset_type *get_cur_fp_regset_address(void) { return &current_fp_regset; }
